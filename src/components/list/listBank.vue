<template>
    <div class="" id="page">
        ...
        <div  class="title-box title-text row">BANQUES</div>
      
      <div id="scrollable-content" class="scrollable-content row">
          <div class="box-titre mt-5">
            <div class="title">Liste des BANQUES</div>
          ...
          <table class="user-table">
            ...
            <tbody>
              <tr v-for="(compte, k) in paginatedComptes" :key="k">
                ...
                <td>{{ compte.user.user_id }}</td>
                <td>{{ compte.user.created_at }}</td>
                <td>{{ compte.user.created_at}}</td>

                <td>
                  <button @click="seeInfoUser(compte.num_ip)" class="action-button">Voir détails</button>
      ...
      <div v-if="see_infoCompte" class="info-modal">
        <h3 class="modal-title">Les infos de {{ choosedUser.user.id }}</h3>
        <button @click="closeInfoModal" class="close-button">&times;</button>
        <table class="info-table">
          <tbody>
            <tr v-for="(value, key, k) in choosedUser" :key="k">
  ...
  export default {
    ...
    data() {
      return {
        ...
        choosedUser: {},
        routeInscription: '/inscription-Banque',
        pageSize: 15,
        currentPage: 1,
      };
    },
    ...
    confirmButtonColor:'orange'
      ...
      seeInfoUser(id) {
        ...
        this.choosedUser = this.allComptes.find(user => user.num_ip === id);
      },
      deleteCompte(compte) {
  ...
  axios.get('https://emploipourtous.africa/api/institutions-financieres')
    .then(response => {
      ...
      const institutionsSansAdminPays = response.data.institutions_financieres.map(institution => {
        const { admin_pays, ...rest } = institution;
        return rest;
      });

      // Mettre à jour allComptes avec les données sans admin_pays
      this.allComptes = institutionsSansAdminPays;
    })
    .catch(error => {
