<template>
    <div id="page">
        ...
        <div id="box-title" style="height: 70px; width: 1950px; background-color: #b9650c">
            <div style="position: absolute; left: 800px; top: 30px;font-size: 20px;font-weight: 900; text-transform: uppercase;">{{status}}s</div> 
        </div>
        <div id="box-liste-rp">
            <div id="box-titre">
                <div style="position: absolute; font-weight: 900; top: 10px; left: 40px">Liste des {{status}}s</div>
            ...
            <div id="liste" style="position: absolute; top: 40px; background-color: transparent; height: 370px; width: 1200px; color: black; overflow: auto">
                <div id="compte" v-for="(compte, k) in allComptes" :key="k" style="position: relative; margin-bottom: 1px; color: black; top: 0px; height: 80px; width: 650px; border-bottom: 1px solid rgb(0, 0, 0); border-radius: 40px">
                    ...
                    <div style="position: absolute; font-weight: 100; top: 40px; left: 50px">[{{compte.zone}}]</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            allComptes: [], see_infoCompte: false, choosedUser: {}, status: 'Front office', routeInscription: '/inscription-frontOffice'
        }
    },
    methods: {
        seeInfoUser(num_ip) {
