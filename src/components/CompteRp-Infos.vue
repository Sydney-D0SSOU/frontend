<template>
    <div id="page">
        ...
        <div id="box-title" style="height: 70px; width: 1950px; background-color: #b9650c">
            <div style="position: absolute; left: 800px; top: 30px;font-size: 20px;font-weight: 900; text-transform: uppercase;">Representants pays</div>
        </div>
        <div id="box-liste-rp">
            <div id="box-titre">
                <div style="position: absolute; font-weight: 900; top: 10px; left: 40px">Liste des representants pays</div>
            ...
            <div id="liste" style="position: absolute; top: 40px; background-color: transparent; height: 370px; width: 1200px; color: black; overflow: auto">
                <div id="compte" v-for="(compte, k) in allComptes" :key="k" style="position: relative; margin-bottom: 1px; color: black; top: 0px; height: 80px; width: 650px; border-bottom: 1px solid rgb(0, 0, 0); border-radius: 40px">
                    <img class="logo" src="../assets/Rp.png" style="top: 10px; left: 10px" alt="Rp">
                    <div @click="seeInfoUser(compte.IFU)" id="seeInfo" style="position: absolute; font-weight: 500; top: 10px; left: 50px">{{compte.designation}}</div>
...
export default {
    ...
    data() {
        return {
            allComptes: [], see_infoCompte: false, choosedUser: {}, status: 'Rp', routeInscription: '/inscription-representant-pays'
        }
    },
    methods: {
        seeInfoUser(IFU) {
            this.see_infoCompte = true;
            this.choosedUser = this.allComptes.find(user => user.IFU === IFU);
        },
        deleteCompte(compte) {
